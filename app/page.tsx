...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-page-bg text-text-main transition-colors duration-500 selection:bg-[#cf3222] selection:text-white">
        ...
        {isSearchOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh] px-4 bg-page-bg/30 backdrop-blur-2xl">
            <motion.div initial={{ scale: 0.99, y: -15 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.99, y: -15 }} className="w-full max-w-2xl bg-card-bg border border-border-color shadow-2xl rounded-[2rem] overflow-hidden">
              ...
              <div className="max-h-[50vh] overflow-y-auto p-3 no-scrollbar text-text-main">
      ...
      {/* --- Navigation (User Name Integration) --- */}
          ...
          <div className="flex items-center gap-10">
            {user ? (
              <div className="flex items-center gap-6">
                ...
                <span className="text-[10px] uppercase tracking-widest font-black text-[#cf3222] italic">
                   {user.email?.split('@')[0]}
                </span>
                <button onClick={() => supabase.auth.signOut()} className="text-[10px] uppercase tracking-widest font-black opacity-40 hover:opacity-100 transition-opacity">Disconnect</button>
              </div>
            ) : (
              <button onClick={() => setIsAuthOpen(true)} className="text-[10px] uppercase tracking-widest font-black opacity-30 hover:opacity-100 transition-opacity">Sign In</button>
            )}
            ...
            {filteredData.map((codex, index) => (
              <motion.div key={codex.name} variants={itemVariants} layout onClick={() => handleModuleClick(codex.url)} className="group block cursor-pointer">
                ...
                <div className="px-3 space-y-3">
                  ...
                  <p className="text-[15px] text-text-muted font-light leading-relaxed max-w-sm line-clamp-2">{codex.desc}</p>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-32 mb-20">
            <div className="space-y-10">
              ...
              <button onClick={() => !user && setIsAuthOpen(true)} className="group flex items-center gap-8 hover:text-[#cf3222] transition-all">
                <span className="text-[11px] font-black tracking-[0.4em] uppercase border-b border-white/10 group-hover:border-[#cf3222] pb-3 transition-all">
                  {user ? "IDENT_VERIFIED" : "INITIALIZE PROTOCOL"}
                </span>
